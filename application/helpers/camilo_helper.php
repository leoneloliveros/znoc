...
if (!function_exists('sumar_restar_hh_mm_ss_a_fecha')) {

    //Funcion para sumar o restar horas, minutosy/o segundos a una fecha
    //recibe 5 parametros; la fecha, el operador "+" o "-", y 3 no requeridos horas, munutos y segundos
    //Retorna la fecha exacta con las horaS sumados o restados
    function sumar_restar_hh_mm_ss_a_fecha($fechaBase, $operador, $horas = 0, $minutos = 0, $segundos = 0) {

        $fecha = date('Y-m-d H:i:s', strtotime($fechaBase));
        $NuevaFecha = strtotime("$operador$horas hour", strtotime($fecha));
        $NuevaFecha = strtotime("$operador$minutos minute", $NuevaFecha);
        $NuevaFecha = strtotime("$operador$segundos second", $NuevaFecha);
...
if (!function_exists('sumar_restar_hh_mm_ss_a_hora')) {

    //Funcion para sumar o restar horas, minutosy/o segundos a una fecha
    //recibe 5 parametros; la fecha, el operador "+" o "-", y 3 no requeridos horas, munutos y segundos
    //Retorna la fecha exacta con las horaS sumados o restados
    function sumar_restar_hh_mm_ss_a_hora($hora_base, $operador, $horas = 0, $minutos = 0, $segundos = 0) {

        $hora = date('H:i:s', strtotime($hora_base));
        $nueva_hora = strtotime("$operador$horas hour", strtotime($hora));
        $nueva_hora = strtotime("$operador$minutos minute", $nueva_hora);
        $nueva_hora = strtotime("$operador$segundos second", $nueva_hora);
...
if (!function_exists('numeros_en_string')) {

    //Funcion para eliminar letras de un string para dejar solo los numeros
    //Recibe solo un argumento que es la cadena a procesar
    // retorna solo los numeros que hayan en la cadena
    function numeros_en_string($string) {
