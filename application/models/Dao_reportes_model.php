...
//        print_r($this->db->last_query().';<br>');
    ...
    public function getMapasCalor($fdesde, $fhasta) {
            $query = $this->db->query("
            	SELECT  IC.TICKETID ,  IC.ZONA_TKT ,  IC.TIPO_TKT ,  IC.CREATIONDATE ,  IC.CLOSEDATE ,  IC.ACTUALFINISH ,  IC.STATUSDATE ,  IC.STATUS ,  IC.DESCRIPTION AS 'DESC' , IC.RUTA_TKT ,  WOR.CREATEDATE, WOR.DESCRIPTION AS 'LONGEST'  FROM maximo.INCIDENT IC
INNER JOIN maximo.WORKLOG WOR ON WOR.RECORDKEY = IC.TICKETID
WHERE (IC.DESCRIPTION LIKE '%PERFOR%' OR IC.DESCRIPTION LIKE '%DEGRADA%SERVIC%' OR IC.DESCRIPTION LIKE '%FUERA%SERVIC%') AND
( IC.RUTA_TKT LIKE '%SERVICIO%MOVI%AFECTA%SERVICI%ACCES%CONTI%' OR IC.RUTA_TKT LIKE '%SERVICIO%MOVI%AFECTA%SERVICI%ACCES%OFENS%AGUD%' OR IC.RUTA_TKT LIKE '%SERVICIO%MOVI%NOTIFIC%AFECT%PERDID%GESTI%' OR IC.RUTA_TKT LIKE '%SERVICIO%MOVI%PERFOR%CALID%DEGRADA%') AND DATE_FORMAT(IC.CREATIONDATE, '%Y-%m-%d') BETWEEN '$fdesde' AND '$fhasta';");
        $data = $query->result();
        $_SESSION['x'] = $data;
        return $data;
    }
    public function getGestionOfensores($fdesde, $fhasta) {
            $query = $this->db->query(" SELECT *
FROM maximo.INCIDENT IC
WHERE (IC.DESCRIPTION LIKE '%RTWP%' OR IC.DESCRIPTION LIKE '%Synchronization%')  and DATE_FORMAT(IC.CREATIONDATE, '%Y-%m-%d') BETWEEN '$fdesde' AND '$fhasta' ;");
        $data = $query->result();
        $_SESSION['x'] = $data;
        return $data;
    }
    public function getTiempoAtencionMC($fdesde, $fhasta) {
            $query = $this->db->query("
            ...
            IC.ACTUALFINISH AS FECHA_CIERRE_INCIDENTE,WO.MODIFYBY AS CREADOR, PER.DISPLAYNAME, WO.CREATEDATE AS FECHA_NOTA,WO.DESCRIPTION AS RESUMEN_NOTA,WO.DESCRIPTION_LONGDESCRIPTION AS DETALLE_NOTA, AL.NODE, AL.NOMB_ALARMA, AL.CREADA, AL.CANCELADA,MONTH(WO.CREATEDATE),
            		DAY(WO.CREATEDATE), IF((DATE(WO.CREATEDATE) = DATE(AC.REPORTDATE)),

                    SEC_TO_TIME(TIMESTAMPDIFF(SECOND, IF(( TIME(AC.REPORTDATE) < '22:00:00'  AND TIME(AC.REPORTDATE) > '06:00:00'),
                         AC.REPORTDATE,
                     CONCAT(DATE(AC.REPORTDATE), ' 06:00:00') ), ( SELECT WOR.CREATEDATE FROM maximo.WORKLOG WOR WHERE WOR.DESCRIPTION LIKE '%INICIO%REVIS%' AND WOR.RECORDKEY = WO.RECORDKEY  LIMIT 1) )), SUBTIME(SEC_TO_TIME(TIMESTAMPDIFF(SECOND, IF(( TIME(AC.REPORTDATE) < '22:00:00'  AND TIME(AC.REPORTDATE) > '06:00:00'),
                         AC.REPORTDATE,
                     CONCAT(DATE(AC.REPORTDATE), ' 06:00:00') ), ( SELECT WOR.CREATEDATE FROM maximo.WORKLOG WOR WHERE WOR.DESCRIPTION LIKE '%INICIO%REVIS%' AND WOR.RECORDKEY = WO.RECORDKEY  LIMIT 1) )), '08:00:00')) tiempo_atencion, SEC_TO_TIME(TIMESTAMPDIFF(SECOND, ( SELECT WOR.CREATEDATE FROM maximo.WORKLOG WOR WHERE WOR.DESCRIPTION LIKE '%INICIO%REVIS%' AND WOR.RECORDKEY = WO.RECORDKEY LIMIT 1 ) , AC.STATUSDATE)) tiempo_diagnostico

        FROM maximo.ACTIVITIES AC
        LEFT JOIN maximo.INCIDENT IC
        ON AC.TICKETID=IC.TICKETID
        LEFT JOIN maximo.ALARMS AL
        ON AL.TICKETID=IC.TICKETID
        LEFT JOIN maximo.WORKLOG WO
        ON AC.WONUM=WO.RECORDKEY
        LEFT JOIN maximo.PERSON PER
        ON PER.PERSONID=WO.MODIFYBY
    ...
    OR AC.OWNER IS NULL) AND WO.DESCRIPTION LIKE '%INIC%REVIS%'
...
//        print_r($this->db->last_query().';<br>');
    ...
    public function getReporteIpRan($fdesde, $fhasta) {
        ...
        END AS NEMO
        FROM maximo.WORKLOG
        WHERE (UPPER(MODIFYBY) LIKE '%ICF6153A%' OR UPPER(MODIFYBY) LIKE '%ECF8833B%' OR UPPER(MODIFYBY) LIKE '%ECF6529C%' OR UPPER(MODIFYBY) LIKE '%ECF2347C%' OR UPPER(MODIFYBY) LIKE '%ECF6895A%' OR UPPER(MODIFYBY) LIKE '%ECF2800B%' OR UPPER(MODIFYBY) LIKE '%EHT4174A%' OR UPPER(MODIFYBY) LIKE '%YULY.ARDILA%' OR UPPER(MODIFYBY) LIKE'%ECF4330C%' OR UPPER(MODIFYBY) LIKE '%EII9470A%' OR UPPER(MODIFYBY) LIKE'%EOY5561A%' OR UPPER(MODIFYBY) LIKE '%ECM9581C'  OR UPPER(MODIFYBY) LIKE '%ECM5250C%' OR UPPER(MODIFYBY) LIKE '%ECM8047A%' OR UPPER(MODIFYBY) LIKE '%ECF5715A%')
