<div class="row">
  <div class="col-sm-12">

    <table border="1">
      <thead>
        <th colspan="24">NEMÓNICOS: <span class="badge" id="totalNemonicos"></span></th>
      </thead>
      <thead>
        <th colspan="4">
          <span>FAOC:</span>&nbsp;&nbsp;<span class="badge" id="FAOCBadge"></span>
        </th>
        <th colspan="4">
          <span>FAOB:</span>&nbsp;&nbsp;<span class="badge" id="FAOBBadge"></span>
        </th>
        <th colspan="4">
          <span>FAPP:</span>&nbsp;&nbsp;<span class="badge" id="FAPPBadge"></span>
        </th>
        <th colspan="4">
          <span>FEE:</span> &nbsp;&nbsp;<span class="badge" id="FEEBadge"></span>
        </th>
        <th colspan="4">
          <span>FI:</span> &nbsp;&nbsp;<span class="badge" id="FIBadge"></span>
        </th>
        <th colspan="4">
          <span>FOIP:</span>&nbsp;&nbsp;<span class="badge" id="FOIPBadge"></span>
        </th>
      </thead>
      <thead>
        <th>
          <span>T1</span>
        </th>
        <th>
          <span>T2</span>
        </th>
        <th>
          <span>T3</span>
        </th>
        <th>
          <span>T11</span>
        </th>
        <th>
          <span>T1</span>
        </th>
        <th>
          <span>T2</span>
        </th>
        <th>
          <span>T3</span>
        </th>
        <th>
          <span>T11</span>
        </th>
        <th>
          <span>T1</span>
        </th>
        <th>
          <span>T2</span>
        </th>
        <th>
          <span>T3</span>
        </th>
        <th>
          <span>T11</span>
        </th>
        <th>
          <span>T1</span>
        </th>
        <th>
          <span>T2</span>
        </th>
        <th>
          <span>T3</span>
        </th>
        <th>
          <span>T11</span>
        </th>
        <th>
          <span>T1</span>
        </th>
        <th>
          <span>T2</span>
        </th>
        <th>
          <span>T3</span>
        </th>
        <th>
          <span>T11</span>
        </th>
        <th>
          <span>T1</span>
        </th>
        <th>
          <span>T2</span>
        </th>
        <th>
          <span>T3</span>
        </th>
        <th>
          <span>T11</span>
        </th>
      </thead>
      <thead>
      <th>
        <span id="T1faoc" class="badge timesT"></span>
      </th>
      <th>
        <span id="T2faoc" class="badge timesT"></span>
      </th>
      <th>
        <span id="T3faoc" class="badge timesT"></span>
      </th>
      <th>
        <span id="T11faoc" class="badge timesT"></span>
      </th>
      <th>
        <span id="T1faob" class="badge timesT"></span>
      </th>
      <th>
        <span id="T2faob" class="badge timesT"></span>
      </th>
      <th>
        <span id="T3faob" class="badge timesT"></span>
      </th>
      <th>
        <span id="T11faob" class="badge timesT"></span>
      </th>
      <th>
        <span id="T1fapp" class="badge timesT"></span>
      </th>
      <th>
        <span id="T2fapp" class="badge timesT"></span>
      </th>
      <th>
        <span id="T3fapp" class="badge timesT"></span>
      </th>
      <th>
        <span id="T11fapp" class="badge timesT"></span>
      </th>
      <th>
        <span id="T1fee" class="badge timesT"></span>
      </th>
      <th>
        <span id="T2fee" class="badge timesT"></span>
      </th>
      <th>
        <span id="T3fee" class="badge timesT"></span>
      </th>
      <th>
        <span id="T11fee" class="badge timesT"></span>
      </th>
      <th>
        <span id="T1fi" class="badge timesT"></span>
      </th>
      <th>
        <span id="T2fi" class="badge timesT"></span>
      </th>
      <th>
        <span id="T3fi" class="badge timesT"></span>
      </th>
      <th>
        <span id="T11fi" class="badge timesT"></span>
      </th>
      <th>
        <span id="T1foip" class="badge timesT"></span>
      </th>
      <th>
        <span id="T2foip" class="badge timesT"></span>
      </th>
      <th>
        <span id="T3foip" class="badge timesT"></span>
      </th>
      <th>
        <span id="T11foip" class="badge timesT"></span>
      </th>
      </thead>
      <tbody>

      </tbody>
    </table>
    <div class="row">
      <div class="col-sm-12">
        <button id="excelVol" class="exportExcel"><i class="far fa-file-excel"></i> Generar Excel</button>
      </div>
    </div>
  </div>
</div>